export const MEMBER_APPS_SCRIPT_SAMPLE = `/**
...
function doPost(e) {
  try {
    ...
    if (action === "sync_all") {
      if (Array.isArray(body.members)) writeMembersSheet(ss, body.members);
      if (Array.isArray(body.externalMembers)) writeExternalMembersSheet(ss, body.externalMembers);
      if (Array.isArray(body.meetings)) writeMeetingsSheet(ss, body.meetings);
      if (Array.isArray(body.progressEntries)) writeProgressSheet(ss, body.progressEntries);
      return responseJSON({
        success: true,
    ...
    if (action === "sync_members_meetings") {
      ...
      return responseJSON({
        success: true,
        message: "已成功同步成員與會議紀錄至 Google Sheets！"
      });
    }
    ...
    if (action === "sync_progress") {
      ...
      return responseJSON({
        success: true,
        message: "已成功同步進度紀錄與在職生資料至 Google Sheets！"
      });
    }
